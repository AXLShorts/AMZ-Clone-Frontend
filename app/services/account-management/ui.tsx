...
export default function AccountManagementClient({ data }: AccountManagementClientProps) {
  if (!data) return null;
  ...
  return (
    <main>
          ...
          <div className="max-w-3xl space-y-8 text-white">
            ...
            >
              <span className="inline-block px-6! py-2.5! bg-brand-accent/20 rounded-full border border-brand-accent/40 backdrop-blur-sm mb-6! text-brand-accent text-sm font-semibold uppercase tracking-wider">
                {data.hero.tagline || "Full Service"}
              </span>
            </motion.div>
      ...
      {data.contentBlock && (
        ...
        >
          {data.contentBlock.gridItems && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-8">
              {data.contentBlock.gridItems.map((item, index) => {
                const IconComponent = (item.icon && (LucideIcons as unknown as Record<string, LucideIcon>)[item.icon]) || LucideIcons.HelpCircle;
                return (
